...
The combined magnitude of the edge strength is calculated by combining the horizontal and vertical gradient magnitudes:

$$
Edge_{\text{strength}}(x, y) = \sqrt{ \left( \sum_{i=-1}^{1} \sum_{j=-1}^{1} G_x(i, j) \cdot I(x+i, y+j) \right)^2 + \left( \sum_{i=-1}^{1} \sum_{j=-1}^{1} G_y(i, j) \cdot I(x+i, y+j) \right)^2 }
$$

where $$\( G_x(i, j) \)$$ and $$\( G_y(i, j) \)$$ are the Prewitt masks and $$\( I(x+i, y+j) \)$$ is the image intensity at the pixel.

#### Truncation

After calculating the edge strength, we truncate the result to a maximum value of 255, ensuring that any computed edge strengths beyond this threshold are set to 255:

$$
Edge_{\text{strength}}(x, y) =
\begin{cases}
255, & \text{if } Edge_{\text{strength}}(x, y) > 255 \\
Edge_{\text{strength}}(x, y), & \text{otherwise}
\end{cases}
$$

#### Final Image Output

The final output image is generated using the truncated edge strengths. The image intensities at each pixel are updated according to the truncated values.

---

### Dynamic Normalization

In this process, we normalize the edge strength by dividing each pixel's edge strength by the maximum edge strength in the image and then scaling it to fit within the range of 0 to 255. This ensures that the maximum edge strength is normalized to 255, enhancing the contrast of the edges.

#### Edge Strength Normalization

The edge strength is normalized by dividing the calculated edge strength by the maximum edge strength value $$\( Max_{\text{value}} \)$$ in the image, and then multiplying by 255:

$$
Edge_{\text{norm}}(x, y) = \frac{Edge_{\text{strength}}(x, y)}{Max_{\text{value}}} \times 255
$$

where $$\( Max_{\text{value}} \)$$ is the maximum edge strength across all pixels in the image:

$$
Max_{\text{value}} = \max_{(x, y)} \left( Edge_{\text{strength}}(x, y) \right)
$$

#### Final Image Output

After normalization, the final image is generated with normalized edge strengths:

$$
I_{\text{final}}(x, y) = Edge_{\text{norm}}(x, y)
$$
...
- **prewitt.v**: This Verilog code calculates the combined edge strength and truncates values above 255.
- **Output Files**:
  - `output_image_combined.jpg`: The resulting image after applying the combined Prewitt operator and thresholding.
  - `output_image_combined.txt`: The raw binary data of the processed image.
