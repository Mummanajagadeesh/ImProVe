...
def text_to_image(file_name):
    input_text_path = f"{file_name}.txt"
    ...
    print(f"Image saved as {output_image_path} with threshold {threshold}")

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python bin2img.py <file_name>")
        sys.exit(1)

    file_name = sys.argv[1]
    text_to_image(file_name)
